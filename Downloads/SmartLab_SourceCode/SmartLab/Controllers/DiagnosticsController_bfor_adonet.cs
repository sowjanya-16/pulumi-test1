...
namespace LC_Reports_V1.Controllers
{
    ...
    public class DiagnosticsController : Controller
    {
        ...
        public ActionResult DiagnosticsView()
        {
            ...
            string Query = " Select RbCode from Sites_AS ";
            ...
            {
                string loc = row["RbCode"].ToString();
                rbcode_AS.Add(loc);
            }
            ...
            if (lstLabDef != null)
            {
                ...
                foreach (var lab in lstLabDef.FindAll(x => x.TypeId == 8 || x.TypeId == 9))
                {
            ...
            try
            {
                ...
                using (BookingServerReplicaEntities db1 = new BookingServerReplicaEntities())
                {
                    foreach (var id in labid_to_pcname)
                    {
                        ...
                        comp = db1.LabComputersPrs.ToList().FindAll(x => x.LabId == id).ToList();
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    ...
                                    var hwItems = (from item in db.Hardware_Desc_Pr where item.Date.Contains(date) && item.System_name.Contains(pcname) select new { item.EEPName, item.Date, item.EEPBuildDate, item.EEPDatabaseVersion, item.LabCarType, item.PCHostName, item.System_name, item.RBCCAFVersion, item.RTPCName, item.RTPCSoftwareVersion, item.ToolChainVersion }).ToList(); 
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    ...
                                    var PrjItems = (from item in db.ProjectDescription_Pr_1 where item.Date.Contains(date) && item.System_name.Contains(pcname) select new { item.AECUCable, item.Ascet_Ver, item.Product, item.Component_name, item.Date, item.Db_Version, item.Details, item.EMUCable, item.MetaEditor_Ver, item.ProjectBuilder_Ver, item.ProjectEditor_Ver, item.System_name, item.ToolVersion, item.Version }).ToList();
        ...
        [HttpPost]
        public ActionResult TABLE2_GetCmpntData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE2_ProjectDescriptionView> PrjDataViews = new List<TABLE2_ProjectDescriptionView>();
            if (pc != null && pc.Count() > 0)
            {
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    ...
                                    var PrjItems = (from item in db.ProjectDescription_Pr_1 where item.Date.Contains(date) && item.System_name.Contains(pcname) select new { item.AECUCable, item.Ascet_Ver, item.Component_name, item.Date, item.Db_Version, item.Details, item.EMUCable, item.MetaEditor_Ver, item.ProjectBuilder_Ver, item.ProjectEditor_Ver, item.System_name, item.ToolVersion, item.Version }).ToList();
        ...
        [HttpPost]
        public ActionResult TABLE3_GetEBData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE3_EBView> table3DataViews = new List<TABLE3_EBView>();
            if (pc != null && pc.Count() > 0)
            {
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    ...
                                    var ebItems = (from item in db.EB_Cards_Pr where item.Date.Contains(date) && item.System_name.Contains(pcname) select new { item.Date, item.Kernel_EB_Cards_EB5100, item.Kernel_EB_Cards_EB5200, item.System_name }).ToList();
        ...
        [HttpPost]
        public ActionResult GetTABLE4_ES4441Data(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE4_ES4441View> table3DataViews = new List<TABLE4_ES4441View>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE5_OTSOData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE5_OTSOView> table5DataViews = new List<TABLE5_OTSOView>();
            if (pc != null && pc.Count() > 0)
            {
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    ...
                                    var otsoItems = (from item in db.OTSO_Pr where item.Date.Contains(date) && item.System_name.Contains(pcname) select new { item.Date, item.Kernel_Component_IPB_TemperatureReceive_OTSO1Available, item.Kernel_Component_IPB_TemperatureReceive_OTSO2Available, item.System_name });
        ...
        [HttpPost]
        public ActionResult GetTABLE6_PowerSupplyData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE6_PowerSupplyView> table6DataViews = new List<TABLE6_PowerSupplyView>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE7_BOBData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE7_BOBView> table3DataViews = new List<TABLE7_BOBView>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE8_BOB1Data(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE8_BOB1View> table4DataViews = new List<TABLE8_BOB1View>();
            if (pc != null && pc.Count() > 0)
            {
                    ...
                    using (PromasterImport_HW_CoGrpsEntities db = new PromasterImport_HW_CoGrpsEntities())
                    {
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    var bob1Items = (from item in db.BreakOutBox_1_Pr where item.System_name.Contains(pcname) && item.Date.Contains(date) select new { item.Date, item.Kernel_Loadbox_Card_ECC_BreakOutBox_BOB_SerialNumber, item.System_name });
        ...
        [HttpPost]
        public ActionResult GetTABLE9_IBData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE9_IBView> table5DataViews = new List<TABLE9_IBView>();
            if (pc != null && pc.Count() > 0)
            {
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    ...
                                    var ibItems = (from item in db.IB_Card_Pr where item.Date.Contains(date) && item.System_name.Contains(pcname) select new { item.Date, item.ModelSpecific_EID_NET_CAN_Settings_CAN_Settings_rtpc_ixxat_config, item.System_name });
        ...
        [HttpPost]
        public ActionResult GetTABLE10_IXXATData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE10_IXXAT2View> table4DataViews = new List<TABLE10_IXXAT2View>();
            if (pc != null && pc.Count() > 0)
            {
                    ...
                    using (PromasterImport_HW_CoGrpsEntities db = new PromasterImport_HW_CoGrpsEntities())
                    {
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    var ixxatItems = (from item in db.IXXAT_Config_Pr where item.Date.Contains(date) && item.System_name.Contains(pcname) select new { item.Date, item.ModelSpecific_EID_NET_CAN_Settings_CAN_Settings_rtpc_ixxat_config, item.System_name });
        ...
        [HttpPost]
        public ActionResult GetTABLE11_APBData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE11_APBView> table3DataViews = new List<TABLE11_APBView>();
            if (pc != null && pc.Count() > 0)
            {
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    ...
                                    var apbItems = (from item in db.APB_Pr where item.Date.Contains(date) && item.System_name.Contains(pcname) select new { item.Date, item.ModelSpecific_EID_Switch_APB_APB_Module_GIO2_Signature_GIO2, item.System_name }).ToList();
        ...
        [HttpPost]
        public ActionResult GetTABLE12_HAPData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE12_HAPView> table4DataViews = new List<TABLE12_HAPView>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE13_HAP1Data(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE13_HAP1View> table3DataViews = new List<TABLE13_HAP1View>();
            if (pc != null && pc.Count() > 0)
            {
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    ...
                                    var hap1Items = (from item in db.Harnesadapter_1_Pr where item.Date.Contains(date) && item.System_name.Contains(pcname) select new { item.Date, item.Kernel_Loadbox_Card_ECC_Harnesadapter_HAP_SerialNumber, item.System_name }).ToList();
        ...
        [HttpPost]
        public ActionResult GetTABLE14_CableData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE14_CableView> table3DataViews = new List<TABLE14_CableView>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE15_Cable1Data(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE15_Cable1View> table4DataViews = new List<TABLE15_Cable1View>();
            if (pc != null && pc.Count() > 0)
            {
                        ...
                        for (var dt = sdate; dt <= edate; dt = dt.AddDays(1))
                        {
                            ...
                            foreach (var pcname in pc)
                            {
                                ...
                                if (pcname != null && pcname != string.Empty && pcname.Count() > 0)
                                {
                                    ...
                                    var cable1Items = (from item in db.Cable_1_Pr where item.Date.Contains(date) && item.System_name.Contains(pcname) select new { item.Date, item.Kernel_Loadbox_Card_ECC_Cable_CABLE_SerialNumber, item.System_name });
        ...
        [HttpPost]
        public ActionResult GetTABLE16_ECCData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE16_ECCView> table5DataViews = new List<TABLE16_ECCView>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE17_GIO1Data(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE6_GIO1View> table6DataViews = new List<TABLE6_GIO1View>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE18_GIO2Data(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE7_GIO2View> table7DataViews = new List<TABLE7_GIO2View>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE19_LDUData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE8_LDUView> table8DataViews = new List<TABLE8_LDUView>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE20_PSCData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE9_PSCView> table9DataViews = new List<TABLE9_PSCView>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE21_VSCData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE10_VSCView> table10DataViews = new List<TABLE10_VSCView>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE22_WSSData(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE11_WSSView> table11DataViews = new List<TABLE11_WSSView>();
            if (pc != null && pc.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetTABLE23_WSS2Data(List<string> pc, DateTime sdate, DateTime edate)
        {
            List<TABLE12_WSS2View> table12DataViews = new List<TABLE12_WSS2View>();
            if (pc != null && pc.Count() > 0)
            {
