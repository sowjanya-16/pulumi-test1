...
namespace LC_Reports_V1.Controllers
{
    public class DashboardController : Controller
    {
        ...
        public ActionResult TSIU_Chart(int id, DateTime startdate, DateTime enddate)
        {
            try
            {
                List<TSIU> labitem = GetLCData(id, startdate, enddate).FindAll(s => s.ID_key == id.ToString());
                if (labitem.Count == 0)
                {
        ...
        public List<TSIU> GetLCData(int id, DateTime StartDate, DateTime EndDate)
        {
            ReportParameters_TSIU objLabReport = new ReportParameters_TSIU();
            Model tempModelObj = new Model();
            TSIU tempTSIUObj = new TSIU();
            ...
            try
            {
                using (BookingServerReplicaEntities db = new BookingServerReplicaEntities())
                {
                    ...
                    if (lstDash != null && lstDash.Count > 0)
                    {
                        ...
                        {
                            objLabReport.LCTSIUParams = new List<TSIU>();
                            ...
                            foreach (LCfilterInfo.ExportManSessSpan manTime in lab.manualActivities)
                            {
                                ...
                                if (dayChangeFlag)
                                {
                                    foreach (LabBookingExportManualSessSpan manspan in templist)
                                    {
                                        ...
                                        tempTSIUObj.endTime = manspan.end.ToUniversalTime();//.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.Id == db.Locations.First(loc => loc.Id == labInfo.LocationId).SiteId).RbCode));
                                        //tempTSIUObj.startTime = LCreportClass.UnixTimeStampsToDate(span.start);
                                        tempTSIUObj.startTime = manspan.start.ToUniversalTime();//.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.Id == db.Locations.First(loc => loc.Id == labInfo.LocationId).SiteId).RbCode));
                                ...
                                else
                                {
                                    ...
                                    //tempTSIUObj.endTime = LCreportClass.UnixTimeStampsToDate(span.end);
                                    tempTSIUObj.endTime = manenddt.ToUniversalTime();//.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.Id == db.Locations.First(loc => loc.Id == labInfo.LocationId).SiteId).RbCode));
                                    //tempTSIUObj.startTime = LCreportClass.UnixTimeStampsToDate(span.start);
                                    tempTSIUObj.startTime = manstartdt.ToUniversalTime();//.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.Id == db.Locations.First(loc => loc.Id == labInfo.LocationId).SiteId).RbCode));
                            ...
                            foreach (LCfilterInfo.ExportAutoSessSpan autoTime in lab.automatedActivities)
                            {
                                ...
                                if (dayChangeFlag)
                                {
                                    foreach (LabBookingExportManualSessSpan autospan in templist)
                                    {
                                        ...
                                        //tempTSIUObj.endTime = LCreportClass.UnixTimeStampsToDate(span.end);
                                        tempTSIUObj.endTime = autospan.end.ToUniversalTime();//.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.Id == db.Locations.First(loc => loc.Id == labInfo.LocationId).SiteId).RbCode));
                                        //tempTSIUObj.startTime = LCreportClass.UnixTimeStampsToDate(span.start);
                                        tempTSIUObj.startTime = autospan.start.ToUniversalTime();//.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.Id == db.Locations.First(loc => loc.Id == labInfo.LocationId).SiteId).RbCode));
                                ...
                                else
                                {
                                    ...
                                    //tempTSIUObj.endTime = LCreportClass.UnixTimeStampsToDate(span.end);
                                    tempTSIUObj.endTime = autoenddt.ToUniversalTime();//.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.Id == db.Locations.First(loc => loc.Id == labInfo.LocationId).SiteId).RbCode));
                                    //tempTSIUObj.startTime = LCreportClass.UnixTimeStampsToDate(span.start);
                                    tempTSIUObj.startTime = autostartdt.ToUniversalTime();//.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.Id == db.Locations.First(loc => loc.Id == labInfo.LocationId).SiteId).RbCode));
    ...
    public class ReportParameters_TSIU
    {
        ...
        private List<TSIU> _LCTSIUField; // if needed
