                            ...
                            foreach (LCfilterInfo.ExportManSessSpan manTime in lab.manualActivities)
                            {
                                ...
                                if (dayChangeFlag)
                                {
                                    foreach (LabBookingExportManualSessSpan manspan in templist)
                                    {
                                        ...
                                        //tempTSIUObj.endTime = manspan.end.ToUniversalTime().AddHours(GetUtcOffsetFromcountryCode(objSites.First(x=>x.RbCode == lab.LabLocation).CountryCode));
                                        ...
                                        //tempTSIUObj.startTime = manspan.start.ToUniversalTime().AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode));
                                ...
                                else
                                {
                                    ...
                                    //tempTSIUObj.endTime = LCreportClass.UnixTimeStampsToDate(span.end);
                                    //tempTSIUObj.endTime = manenddt.ToUniversalTime().AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode));
                                    tempTSIUObj.endTime = manenddt.ToUniversalTime();
                                    //tempTSIUObj.startTime = LCreportClass.UnixTimeStampsToDate(span.start);
                                    tempTSIUObj.startTime = manstartdt.ToUniversalTime();
                                    //tempTSIUObj.startTime = manstartdt.ToUniversalTime().AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode));
                            ...
                            foreach (LCfilterInfo.ExportAutoSessSpan autoTime in lab.automatedActivities)
                            {
                                ...
                                if (dayChangeFlag)
                                {
                                    foreach (LabBookingExportManualSessSpan autospan in templist)
                                    {
                                        ...
                                        //tempTSIUObj.endTime = LCreportClass.UnixTimeStampsToDate(span.end);
                                        //tempTSIUObj.endTime = autospan.end.ToUniversalTime().AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode));
                                        tempTSIUObj.endTime = autospan.end.ToUniversalTime();
                                        //tempTSIUObj.startTime = LCreportClass.UnixTimeStampsToDate(span.start);
                                        tempTSIUObj.startTime = autospan.start.ToUniversalTime();
                                        //tempTSIUObj.startTime = autospan.start.ToUniversalTime().AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode));
                                ...
                                else
                                {
                                    ...
                                    //tempTSIUObj.endTime = LCreportClass.UnixTimeStampsToDate(span.end);
                                    //tempTSIUObj.endTime = autoenddt.ToUniversalTime().AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode));
                                    ...
                                    //tempTSIUObj.startTime = autostartdt.ToUniversalTime().AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode));
