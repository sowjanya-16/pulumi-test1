...
namespace LC_Reports_V1.Controllers
{
    ...
    public class LabCarController : Controller
    {
        ...
        public ActionResult Lab_Report(bool reload = false)
        {
            ...
            try
            {
                using (BookingServerReplicaEntities db = new BookingServerReplicaEntities())
                {
                    ...
                    lcfilterInfo.endDate = DateTime.Now.Date;
                            ...
                            foreach (LCfilterInfo.ExportManSessSpan manTime in lab.manualActivities)
                            {
                                ...
                                if (dayChangeFlag)
                                {
                                    foreach (LabBookingExportManualSessSpan manspan in templist)
                                    {
                                        ...
                                        //tempTSIUObj.endTime = manspan.end.ToUniversalTime().ToString(); // chnaged from this to next line because of timezones for each country.                                      

                                        //tempTSIUObj.endTime = manspan.end.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x=>x.RbCode == lab.LabLocation).CountryCode)).ToString();
                                        ...
                                        //tempTSIUObj.startTime = manspan.start.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode)).ToString();
                                        //tempTSIUObj.startTime = manspan.start.ToUniversalTime().ToString();
                                ...
                                else
                                {
                                    ...
                                    //tempTSIUObj.endTime = LCreportClass.UnixTimeStampsToDate(span.end);
                                    //tempTSIUObj.endTime = manenddt.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode)).ToString();
                                    //tempTSIUObj.endTime = manenddt.ToUniversalTime().ToString();
                                    //tempTSIUObj.startTime = LCreportClass.UnixTimeStampsToDate(span.start);
                                    //tempTSIUObj.startTime = manstartdt.ToUniversalTime().ToString();
                                    //tempTSIUObj.startTime = manstartdt.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode)).ToString();
                            ...
                            foreach (LCfilterInfo.ExportAutoSessSpan autoTime in lab.automatedActivities)
                            {
                                ...
                                if (dayChangeFlag)
                                {
                                    foreach (LabBookingExportManualSessSpan autospan in templist)
                                    {
                                        ...
                                        //tempTSIUObj.endTime = LCreportClass.UnixTimeStampsToDate(span.end);
                                        //tempTSIUObj.endTime = autospan.end.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode)).ToString();
                                        //tempTSIUObj.endTime = autospan.end.ToUniversalTime().ToString();
                                        //tempTSIUObj.startTime = LCreportClass.UnixTimeStampsToDate(span.start);
                                        //tempTSIUObj.startTime = autospan.start.ToUniversalTime().ToString();
                                        //tempTSIUObj.startTime = autospan.start.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode)).ToString();
                                ...
                                else
                                {
                                    ...
                                    //tempTSIUObj.endTime = LCreportClass.UnixTimeStampsToDate(span.end);
                                    //tempTSIUObj.endTime = autoenddt.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode)).ToString();
                                    //tempTSIUObj.endTime = autoenddt.ToUniversalTime().ToString();
                                    //tempTSIUObj.startTime = LCreportClass.UnixTimeStampsToDate(span.start);
                                    //tempTSIUObj.startTime = autostartdt.ToUniversalTime().ToString();
                                    ...
                                    //tempTSIUObj.startTime = autostartdt.AddHours(GetUtcOffsetFromcountryCode(objSites.First(x => x.RbCode == lab.LabLocation).CountryCode)).ToString();
        ...
        public ActionResult TSOU_Chart(int[] labIdvalue, string startdate, string enddate)
        {
            ...
            LabBookingExport objLabExport = new LabBookingExport();
            ...
            using (BookingServerReplicaEntities db = new BookingServerReplicaEntities())
            {
                ...
                objLabExport.Labs = new LabBookingExportLab[labIdvalue.Length];
                ...
                try
                {
                    ...
                    {
                        objLabExport.Labs[labcount] = new LabBookingExportLab();
                        List<LabComputersPr> labComputers = new List<LabComputersPr>();
