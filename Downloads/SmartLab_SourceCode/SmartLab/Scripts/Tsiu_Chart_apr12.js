...
var man_caplhrs;
var auto_caplhrs;
function generateChartTSIU(data) {
    debugger;
    ...
    if (data.length == 0) {
        var url = "Dashboard/Dashboard_Options";
    ...
    for (i = 0; i < data.length; i++) {
        excelobjtsiu[i].LC_Name = $("#selectid option:selected").html();
       ...
       // debugger;
        ...
        excelobjtsiu[i].s_day = weekday[StDate.getDay()].toString();
        
        var Time1 = StDate.toTimeString();
        var time = Time1.match(/(\d+):(\d+):(\d+) (\w)/);
        ...
        excelobjtsiu[i].Startdate = mm1 + '/' + dd1 + '/' + yyyy1;
        ...
        excelobjtsiu[i].e_day = weekday[EndDate.getDay()].toString();

        var Time2 = EndDate.toTimeString();
        var time = Time2.match(/(\d+):(\d+):(\d+) (\w)/);
        var hours = Number(time[1]);
        var minutes = Number(time[2]);
        var seconds = Number(time[3]);
        var meridian = time[4].toLowerCase();
        ...
        excelobjtsiu[i].Enddate = mm1 + '/' + dd1 + '/' + yyyy1;
        
        excelobjtsiu[i].endTime = hours + ":" + minutes + ":" + seconds;
        //time difference in hours : start
        var diff = (EndDate.getTime() - StDate.getTime()) / 1000;
        diff /= (60 * 60);
        excelobjtsiu[i].diffTime = diff.toFixed(2);
        //end
        ...
        var strEndDay = excelobjtsiu[i].Startdate;
        ...
        if (!dayChangeFlag) {
            ...
            var daydiff = (Math.abs(edDate - stDate)) / (1000 * 3600 * 24);
            ...
            do {
                ...
                else if (daydiff == Math.ceil((Date.parse(EndDateTime)) - (Date.parse(StartDateTime)))) {
                    chart.data[cnt] = {
                        "days": (excelobjtsiu[i].s_day + "," + excelobjtsiu[i].Startdate),
                        "start": excelobjtsiu[i].startTime,
                        "end": "23:59:59",
...
$("#export2").click(function () {
    ...
    $.ajax({
        type: "POST",
        url: "/Dashboard/ExportTSIUDataToExcel/",
        ...
        error: function () {
            ...
            $.notify(' Error in Exproting TSIU Data to an Excel Sheet. Please try again Later.', {
                globalPosition: "top center",
                className: "warn"
            });
        }
