...
function mySuccessFuntion(rdata) {
    ...
    if (rdata.success) {
        ...
        function success_refresh_getdata(response) {
            var objdata_rfoview = (response.data.filter(item => item.LinkedRequestID == ""));
            ChildList = (response.data.filter(item => item.LinkedRequestID != ""));
            $("#RequestTable_RFO").dxDataGrid({ dataSource: objdata_rfoview });
        }
...
`),
                            ...
                            "edit",//, "delete",
                        {
                            ...
                            onClick: function (e) {
                                debugger;
                                //IF MB - CC -> FILE UPLOAD OPTION IS NOT PROVIDED TO THE USERS - VALIDATED WITH GRID EDIT FLAG AS IT WOULD BE TRUE FOR CC;
                                //IF BGSW -> FILE MUST BE UPLOADED BEFORE SUBMIT
                                if (gridedit || e.row.data.Upload_File_Name.toString().length > 0) {
                                    debugger;
...
function Delete(id, filtered_yr) {
    ...
    $.ajax({
        ...
        data: { 'id': id, 'useryear': filtered_yr },
        success: function (data) {
            var objdata_rfoview = (data.data.filter(item => item.LinkedRequestID == ""));
            ChildList = (data.data.filter(item => item.LinkedRequestID != ""));
            $("#RequestTable_RFO").dxDataGrid({ dataSource: objdata_rfoview });
        }
